...
# SPECIFICATION: To output the rules that satisfy minsup = 0.2 and minconf = 0.6,
# as well as the priors and probability gains of the rule consequents when
# conditioned to the antecedents.
...
for index, row in df.iterrows():
    for index, row in df.iterrows():
        labels = {'Bagel': 0, 'Bread': 0, 'Cheese': 0, 'Diaper': 0, 'Eggs': 0, 'Meat': 0, 'Milk': 0, 'Pencil': 0, 'Wine': 0}
        for a in row:
            if a is not np.nan:
                labels[a] = 1
...
for index, row in rules.iterrows():
    print(str(list(row['antecedents'])) + ' -> ' + str(list(row['consequents'])))
    print('Support: ' + str(row['support']))
    print('Confidence: ' + str(row['confidence']))
    supportCount = row['consequent support']*len(encoded_vals)
    prior = supportCount/len(encoded_vals)
    print('Prior: ' + str(prior))
    print("Gain in Confidence: " + str(100*(row['confidence'])/prior))
